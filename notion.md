...
| 總市值 | Rollup | - Relation: `股票代號` <br> - Property: `市值` <br> - Calculate: `Sum` | 股票代號．市值 |
| 總資產 | Formula | `總市值 + 剩餘現金` | 總市值、剩餘現金 |
| 資產高點 | Number | 手動由 Button 更新，記錄歷史最高總資產 | — |
| 資產回撤率 | Formula | 總資產相對資產高點的漲跌幅 | 總資產、資產高點 |
| 創新高 | Formula | 總資產是否超過資產高點 | 總資產、資產高點 |
| 更新資產高點 | Button | 點擊後更新資產高點為當前總資產與現有高點的最大值 | 總資產、資產高點 |
| 股票代號 | Relation | 關聯至股票代號 Database | — |
...
```jsx
if(
  not empty(prop("資產高點")) and prop("資產高點") > 0,
  (prop("總資產") - prop("資產高點")) / prop("資產高點"),
  empty()
)
```

> Number format 設為 Percent。正數代表創新高，負數代表回撤。
...
```jsx
max(
  if(empty(prop("總資產")), 0, prop("總資產")),
  if(empty(prop("資產高點")), 0, prop("資產高點"))
)
```
...
資產 Database
  ├── 總市值（from 股票代號．市值）
  ├── 剩餘現金（from 總投入金額 + 總淨收付合計）
  ├── 總資產（from 總市值 + 剩餘現金）
  ├── 資產高點（from Button）
  ├── 資產回撤率（from 總資產 vs 資產高點）
  ├── 創新高（from 總資產 vs 資產高點）
  └── 更新資產高點（Button）
```
